...
async function ensureIndexes(db: Db) {
  await Promise.all([
    ...
    db.collection('board_cards').createIndex({ org_id: 1, due_date: 1, deleted_at: 1 }),
    db.collection('board_cards').createIndex({ org_id: 1, defer_until: 1, deleted_at: 1 }),
    db.collection('board_cards').createIndex({ org_id: 1, start_date: 1, deleted_at: 1 }),
